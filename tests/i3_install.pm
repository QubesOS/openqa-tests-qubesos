...
#    assert_script_run('/bin/cp -a /root/extra-files/system-tests /srv/salt/');
    ...
    assert_script_run("(set -o pipefail; sudo qubes-dom0-update -y i3 i3-settings-qubes 2>&1 | tee /tmp/i3-install.log)", timeout => 1800);
    $self->maybe_unlock_screen;
    upload_logs('/tmp/i3-install.log', failok => 1);
