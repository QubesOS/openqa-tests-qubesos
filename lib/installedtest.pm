...
sub handle_system_startup {
    ...
    if (check_var("MACHINE", "hw7") or check_var("MACHINE", "hw12") or check_var("MACHINE", "hw16") or check_var("MACHINE", "hw5")) {
...
sub connect_wifi {
    ...
    if (!check_screen("nm-applet-connected-wifi", 120)) {
        ...
        assert_and_click("nm-applet-more-networks", mousehide => -1);
        assert_and_click($wifi_needle, mousehide => 1);
        # network list refresh can hit just before clicking, retry in that case
        if (!check_screen("nm-applet-wifi-password", 60)) {
            assert_and_click("nm-applet-more-networks", mousehide => -1);
            assert_and_click($wifi_needle, mousehide => 1);
        }
...
sub post_fail_hook {
    ...
    script_run "tail -500 /var/log/xen/console/guest-sys-net.log";
