...
curl() {
    ...
    if [ -n "\$inputfile" ]; then
        qvm-run --no-gui -p sys-net "curl --max-time 15 \$allargs" <\$inputfile
    else
...
ENDFUNC
    ...
    assert_script_run("qvm-run --no-gui -p -u root sys-net \"command -v curl || (apt-get update --allow-releaseinfo-change && apt-get -y install curl ) || dnf install -y curl\" </dev/null", timeout => 120);
    assert_script_run("qvm-run --no-gui -p sys-net \"curl -f " . autoinst_url('/files/curl-wrapper.sh') . "\" </dev/null > curl-wrapper.sh");
