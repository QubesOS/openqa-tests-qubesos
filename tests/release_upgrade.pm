...
sub upgrade_42_to_43_before_reboot {
    assert_script_run('ln -nsf /var/log/qubes-dist-upgrade.log release-upgrade.log');
    assert_script_run("qvm-service --disable dom0 qubes-update-check");
    assert_script_run("sudo qubes-dist-upgrade --releasever=4.3 --assumeyes --update --max-concurrency=1 --enable-current-testing", timeout => 7200);
    assert_script_run("sudo qubes-dist-upgrade --releasever=4.3 --assumeyes --release-upgrade --enable-current-testing", timeout => 600);
    assert_script_run("sudo qubes-dist-upgrade --releasever=4.3 --assumeyes --dist-upgrade --enable-current-testing", timeout => 7200);
    sleep(1);
    send_key('ctrl-c');
...
sub upgrade_42_to_43_after_reboot {
    ...
    assert_script_run('ln -nsf /var/log/qubes-dist-upgrade.log release-upgrade-post-reboot.log');
    assert_script_run("sudo qubes-dist-upgrade --releasever=4.3 --assumeyes --template-standalone-upgrade --enable-current-testing", timeout => 8000);
    $self->maybe_unlock_screen;
    ...
    $self->maybe_unlock_screen;
    assert_script_run("sudo qubes-dist-upgrade --releasever=4.3 --assumeyes --finalize --enable-current-testing", timeout => 7200);
    $self->maybe_unlock_screen;
}
